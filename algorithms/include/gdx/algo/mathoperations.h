...
RasterType<ResultType> perform_operation(const RasterType<T>& input, Op&& oper)
{
...
template <typename ResultType, template <typename> typename RasterType, typename T, typename Op>
RasterType<ResultType> perform_operation(const RasterType<T>& input1, const RasterType<T>& input2, Op&& oper)
{
...
{
    return internal::perform_operation<float>(ras, [](T value) {
...
{
    return internal::perform_operation<double>(ras, [](double value) {
...
{
    if constexpr (std::is_same_v<double, T>) {
        return internal::perform_operation<double>(ras1, ras2, [](T value1, T value2) {
            return std::pow(value1, value2);
        });
    } else {
        return internal::perform_operation<float>(ras1, ras2, [](T value1, T value2) -> float {
...
template <template <typename> typename RasterType, typename T>
auto pow(const RasterType<T>& ras1, double value)
{
    if constexpr (std::is_same_v<double, T>) {
        return internal::perform_operation<double>(ras1, [value](T value1) {
            return std::pow(value1, value);
        });
    } else {
        return internal::perform_operation<float>(ras1, [value](T value1) -> float {
            return static_cast<float>(std::pow(value1, value));
        });
    }
}
...
template <template <typename> typename RasterType, typename T>
void clip_low(const RasterType<T>& input, RasterType<T>& output, T low)
{
...
template <template <typename> typename RasterType, typename T>
void clip_high(const RasterType<T>& input, RasterType<T>& output, T high)
{
...
template <template <typename> typename RasterType, typename T>
RasterType<T> clip_low(const RasterType<T>& input, T low)
{
    RasterType<T> output(input.metadata());
    clip_low(input, output, low);
    return output;
}

template <template <typename> typename RasterType, typename T>
RasterType<T> clip_high(const RasterType<T>& input, T high)
{
    RasterType<T> output(input.metadata());
    clip_high(input, output, high);
    return output;
}
}
