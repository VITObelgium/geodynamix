...
TEST_CASE_TEMPLATE("ClusterSize", TypeParam, RasterTypes)
{
    SUBCASE("cluster size")
    {
        ...
        auto actual = cluster_size(ras, ClusterDiagonals::Exclude);
        CHECK_RASTER_EQ(expected, actual);
    }

    SUBCASE("clusterSizeBorderValues")
    {
    ...
    SUBCASE("clusterSizeResultis_nodataValue")
    {
        ...
        auto actual = cluster_size(ras, ClusterDiagonals::Exclude);
        CHECK_RASTER_EQ(expected, actual);
        CHECK_FALSE(expected.is_nodata(0, 0));
    }
}
}