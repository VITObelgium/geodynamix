...
RasterType voronoi(const RasterMetadata& meta, std::span<const Cell> locations)
{
    ...
    if constexpr (!std::is_floating_point_v<T>) {
        if (locations.size() > static_cast<size_t>(std::numeric_limits<T>::max())) {
            throw InvalidArgument("More locations provided then possibe for the data type: {} vs {}", locations.size(), std::numeric_limits<T>::max());
        }
    }
