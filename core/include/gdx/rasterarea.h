...
public:
    ...
    auto begin()
    {
        return RasterIterator<value_type, is_const, ValueFilterPolicy, LocationFilterPolicy, ProxyType>(_rasterTopLeft, _rows, _cols, _stride, _areaTopLeft, _valueFilter, _locationFilter);
    }

    auto end()
    {
        return RasterIterator<value_type, is_const, ValueFilterPolicy, LocationFilterPolicy, ProxyType>();
    }
...
auto neighbouring_cells_square(Container&& raster, Cell cell, int32_t radius)
{
    ...
    return RasterArea<T, isConst, NodataValueFilterPolicy<T>, SingleCellLocationFilterPolicy, ValueProxy<T, isConst>>(raster.data(), areaInfo.topLeftPtr, areaInfo.rows, areaInfo.cols, raster.cols(), valueFilter, locationFilter);
}
...
auto neighbouring_cells_square(Container&& raster, Cell center, int32_t radius)
{
    ...
    NodataMaskFilterPolicy<T, TMask, isConst> valueFilter(raster.data(), raster.mask());
    SingleCellLocationFilterPolicy locationFilter(center);
    return RasterArea<T, isConst, NodataMaskFilterPolicy<T, TMask, isConst>, SingleCellLocationFilterPolicy, MaskValueProxy<T, isConst>>(raster.data(), areaInfo.topLeftPtr, areaInfo.rows, areaInfo.cols, raster.cols(), valueFilter, locationFilter);
}
...
auto cells_square(Container&& raster, Cell cell, int32_t radius)
{
    ...
    auto areaInfo = detail::clip_area_to_raster(raster, cell, radius);
    NodataValueFilterPolicy<T> valueFilter(raster.nodata());
    return RasterArea<T, isConst, NodataValueFilterPolicy<T>, AllLocationsFilterPolicy, ValueProxy<T, isConst>>(raster.data(), areaInfo.topLeftPtr, areaInfo.rows, areaInfo.cols, raster.cols(), valueFilter, AllLocationsFilterPolicy());
}
...
auto cells_square(Container&& raster, Cell center, int32_t radius)
{
    ...
    auto areaInfo = detail::clip_area_to_raster(raster, center, radius);
    NodataMaskFilterPolicy<T, TMask, isConst> valueFilter(raster.data(), raster.mask());
    return RasterArea<T, isConst, NodataMaskFilterPolicy<T, TMask, isConst>, AllLocationsFilterPolicy, MaskValueProxy<T, isConst>>(raster.data(), areaInfo.topLeftPtr, areaInfo.rows, areaInfo.cols, raster.cols(), valueFilter, AllLocationsFilterPolicy());
}
...
auto neighbouring_cells_circular(Container&& raster, Cell center, int radius)
{
    ...
    return RasterArea<T, isConst, NodataValueFilterPolicy<T>, CircularLocationFilterPolicy<CenterCellHandling::Exclude>, ValueProxy<T, isConst>>(
        raster.data(),
        areaInfo.topLeftPtr,
        areaInfo.rows,
        areaInfo.cols,
...
auto neighbouring_cells_circular(Container&& raster, Cell center, int radius)
{
    ...
    return RasterArea<T, isConst, NodataMaskFilterPolicy<T, TMask, isConst>, CircularLocationFilterPolicy<CenterCellHandling::Exclude>, MaskValueProxy<T, isConst>>(
        raster.data(),
        areaInfo.topLeftPtr,
        areaInfo.rows,
        areaInfo.cols,
...
template <
    typename Container,
    typename std::enable_if_t<std::decay_t<Container>::with_nodata, int>* = nullptr>
auto cells_circular(Container&& raster, Cell center, int radius)
{
    using T                = typename std::decay_t<Container>::value_type;
    constexpr bool isConst = std::is_const_v<std::remove_reference_t<Container>>;
    ...
    auto areaInfo = detail::clip_area_to_raster(raster, center, radius);
    NodataValueFilterPolicy<T> valueFilter(raster.nodata());
    CircularLocationFilterPolicy<CenterCellHandling::Include> locationFilter(center, radius);

    return RasterArea<T, isConst, NodataValueFilterPolicy<T>, CircularLocationFilterPolicy<CenterCellHandling::Include>>(
        raster.data(),
        areaInfo.topLeftPtr,
        areaInfo.rows,
        areaInfo.cols,
...
auto cells_circular(Container&& raster, Cell center, int radius)
{
    ...
    return RasterArea<T, isConst, NodataMaskFilterPolicy<T, TMask, isConst>, CircularLocationFilterPolicy<CenterCellHandling::Include>, MaskValueProxy<T, isConst>>(
        raster.data(),
        areaInfo.topLeftPtr,
        areaInfo.rows,
        areaInfo.cols,
...
template <
    typename Container,
    typename std::enable_if_t<std::decay_t<Container>::with_nodata, int>* = nullptr>
auto sub_area_values(Container&& raster, const RasterMetadata& subExtent)
{
    auto topLeft = raster.metadata().convert_point_to_cell(subExtent.top_left());
    return sub_area_values(raster, topLeft, subExtent.rows, subExtent.cols);
}
...
auto sub_area_values(Container&& raster, Cell topLeft, int32_t rows, int32_t cols)
{
    ...
    auto areaInfo = detail::clip_area_to_raster(raster, topLeft, rows, cols);
    return RasterArea<T, isConst, NodataMaskFilterPolicy<T, TMask, isConst>, AllLocationsFilterPolicy, MaskValueProxy<T, isConst>>(
        raster.data(),
        areaInfo.topLeftPtr,
        areaInfo.rows,
        areaInfo.cols,
        raster.cols(),
        NodataMaskFilterPolicy<T, TMask, isConst>(raster.data(), raster.mask()),
        AllLocationsFilterPolicy());
}
}
