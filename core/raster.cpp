...
{
    if (type() != other.type()) {
        Log::warn("rasters unequal because types are different");
        return false;
    }
...
{
    return read(fileName, inf::gdal::io::get_raster_type(fileName));
}

Raster Raster::read(const fs::path& fileName, const RasterMetadata& extent)
{
    return read(fileName, inf::gdal::io::get_raster_type(fileName), extent);
}
