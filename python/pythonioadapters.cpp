...
static RasterMetadata read_metadata(py::object path)
{
    return inf::gdal::io::read_metadata(handle_path(path));
}
...
static std::tuple<RasterMetadata, py::array> readArray(py::object path, py::object dtype, const RasterMetadata* extent)
{
    ...
    const auto& type = [&]() -> const std::type_info& {
        if (dtype.is_none()) {
            return inf::gdal::io::get_raster_type(fsPath);
        } else {
...
static void writeArray(py::array data, const RasterMetadata& meta, py::object path, py::object dtype)
{
    const auto& type = [&]() -> const std::type_info& {
        if (dtype.is_none()) {
            return inf::gdal::io::get_raster_type(handle_path(path));
        } else {
            return dtypeToRasterType(py::dtype::from_args(dtype));
        }
    }();
    ...
    std::span<const uint8_t> dataSpan(reinterpret_cast<const uint8_t*>(data.data()), data.size());
    inf::gdal::io::write_raster(dataSpan, meta, handle_path(path), type);
}
