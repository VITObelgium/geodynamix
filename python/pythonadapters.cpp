...
py::list rasterBounds(const RasterMetadata& meta, bool projected)
{
    ...
    if (meta.projection.empty()) {
        ...
        topLeftCorner     = inf::gdal::projected_to_geographic(sourceEpsg.value(), topLeftCorner);
        bottomRightCorner = inf::gdal::projected_to_geographic(sourceEpsg.value(), bottomRightCorner);
    }
...
void writeAsPng(const RasterType& ras, const RasterMetadata& meta, const fs::path& filename, py::object colorMap, bool normalize)
{
    ...
    if (colorMap.is_none()) {
        if (normalize) {
            ...
            inf::gdal::io::write_raster(std::span<const uint8_t>(clippedData), clippedMeta, filename);
        } else {
...
py::object showRaster(py::object rasterArg, py::object colorMap, bool normalize)
{
    return std::visit([&](auto&& raster) {
        ...
        if (hasProjectionInfo) {
            workRaster    = std::make_unique<RasterT>(gdx::warp_raster(raster, crs::epsg::WGS84WebMercator));
